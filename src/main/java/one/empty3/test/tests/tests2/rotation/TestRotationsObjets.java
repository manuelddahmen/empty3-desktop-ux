...
package one.empty3.test.tests.tests2.rotation;
...
public class TestRotationsObjets extends TestObjetSub {
    ...
    public TestRotationsObjets(double globalTimeMillis) {
        ...
        try {
            imageTexture = new TextureImg(new Image(ImageIO.read(this.getClass().getResourceAsStream("map2.png"))));
        } catch (IOException e) {
            e.printStackTrace();
    ...
    protected Representable initObjet(int no) {
        switch (no) {
            case 0:
                representable = new LineSegment(Point3D.Y.mult(-dim), Point3D.Y.mult(dim), new TextureCol(java.awt.Color.BLACK));
