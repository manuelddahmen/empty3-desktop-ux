...
public final class MainPanel extends JPanel {
    ...
    private static void initBreadcrumbList(Container p, JTree tree) {
        ...
        for (int i = 0; i < paths.length; i++) {
            AbstractButton b = makeButton(tree, copyTreePath(paths, i + 1), new Color(Color.ORANGE));
            p.add(b);
            bg.add(b);
        }
    }
    ...
    private static Component makeBreadcrumbList(List<String> list) {
        ...
        ButtonGroup bg = new ButtonGroup();
        list.forEach(title -> {
            AbstractButton b = makeButton(null, new TreePath(title), new Color(Color.PINK));
            p.add(b);
            bg.add(b);
        });
        return p;
...
class ArrowToggleButtonBarCellIcon implements Icon {
    ...
    public void paintIcon(Component c, Graphics g, int x, int y) {
        ...
        Color bgc = new Color(parent.getBackground());
        Color borderColor = new Color(Color.GRAY.brighter());
