            ...
            for (int i = 0; i < grid.getData2d().size(); i++) {
                for (int j = 0; j < grid.getData2d().get(i).size(); j++) {
                    ...
                    if (point3D.equals(selectedPoint)) {
                        sphere.texture(new ColorTexture(Color.RED.getRGB()));
                    } else {
                        sphere.texture(new ColorTexture(Color.BLUE.getRGB()));
                    }
                    if (grid.inBounds(i + 1, j)) {
                        rc.add(new LineSegment(point3D, grid.getElem(i + 1, j), new ColorTexture(Color.GREEN.getRGB())));
                    }
                    if (grid.inBounds(i, j + 1)) {
                        rc.add(new LineSegment(point3D, grid.getElem(i, j + 1), new ColorTexture(Color.GREEN.getRGB())));
                    }
                    sphere.setIncrU(0.4);
                    sphere.setIncrV(0.4);
                    rc.add(sphere);
