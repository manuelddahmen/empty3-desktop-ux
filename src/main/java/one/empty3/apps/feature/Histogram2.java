...
; i++) {
                for (int j = 0; j < m.getLines(); j++) {
                    ...
                    while (level.i >= 0 && level.i < 1.0 && level.i >= min[index0] && level.i < max[index0] && rMin < Math.max(m.getColumns()
, m.getLines())) {
        ...
        try {
            ...
            pointsOfInterest.stream().forEach(circle -> {
                if (circle.i >= min && circle.r > 1.0) {
                        ...
                        */
                    Color color = new Color(Lumiere.getIntFromFloats((float) (circle.i < 1.0 ? (circle.i) : 1f), 0f, (float) (circle.i / circle.r < 1.0
            ...
            // grouper les points par similarites et distances
              /*  group(pointsOfInterest);
                ...
                File fileToWrite3 = new File(directory.getAbsolutePath()
                        + "level"+ "_NEW_RGB.jpg");
                //fileToWrite.mkdirs();*/
            new one.empty3.libs.Image(new PixM(img3).normalize(0., 1.).getImage()).saveFile(out);
                /*
