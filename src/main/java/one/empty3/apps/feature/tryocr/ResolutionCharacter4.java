        ...
        if (dir.exists() && dir.isDirectory()) {
            ...
            for (File file : Objects.requireNonNull(dir.listFiles())) {
                if (!file.isDirectory() && file.isFile() && file.getName().toLowerCase(Locale.ROOT).endsWith(".jpg")) {
                    ...
                    Logger.getAnonymousLogger().log(Level.INFO, "ResolutionCharacter4 : " + name);
    ...
    private void exec(int i, int j) {
        if (arrayDiff(input.getValues(i, j), WHITE_DOUBLES) < MIN_DIFF) {
            ...
            if (succeded) {
                ...
                List<Character> candidates = recognize(input, new one.empty3.apps.feature.tryocr.Rectangle2(rectangle));
        ...
        if (arrayDiff(input.getValues(i, j), WHITE_DOUBLES) < MIN_DIFF) {
            ...
            if (succeded) {
                ...
                if (reduce(input, new one.empty3.apps.feature.tryocr.Rectangle2(rectangle), rectangle2)) {
                    rectangle2 = new one.empty3.apps.feature.tryocr.Rectangle2(rectangle);
