...
public class ResolutionCharacter8 implements Runnable {
    ...
    PixM outRecompose;
    HashMap<Character, one.empty3.apps.feature.tryocr.Letter> letters = new HashMap<Character, one.empty3.apps.feature.tryocr.Letter>();
    ...
    private List<one.empty3.apps.feature.tryocr.Rectangle2> rectangles;
    private List<one.empty3.apps.feature.tryocr.Rectangle2> rectangles2;
...

    {
        letters.put('a', new one.empty3.apps.feature.tryocr.Letter(
                new one.empty3.apps.feature.tryocr.Trait(one.empty3.apps.feature.tryocr.TraitsY.Up, one.empty3.apps.feature.tryocr.TraitsX.Left, one.empty3.apps.feature.tryocr.TraitsY.Up, one.empty3.apps.feature.tryocr.TraitsX.Right,
                        one.empty3.apps.feature.tryocr.TraitsShape.Round, one.empty3.apps.feature.tryocr.TraitsShape.Round),
                new Trait(one.empty3.apps.feature.tryocr.TraitsY.Up, one.empty3.apps.feature.tryocr.TraitsX.Right, TraitsY.Down, TraitsX.Right,
                        one.empty3.apps.feature.tryocr.TraitsShape.Round, TraitsShape.Line)));
    }
    ...
    private void deleteEquals(List<one.empty3.apps.feature.tryocr.Rectangle2> rectangles) {
        for (int i = 0; i < rectangles.size(); i++) {
            one.empty3.apps.feature.tryocr.Rectangle2 rect1 = rectangles.get(i);
            one.empty3.apps.feature.tryocr.Rectangle2 delete;
            for (int j = i; j < rectangles.size(); ) {
                one.empty3.apps.feature.tryocr.Rectangle2 rect2 = rectangles.get(j);
    ...
    private void compare(List<one.empty3.apps.feature.tryocr.Rectangle2> rectangles) {
        PixM distances = new PixM(rectangles.size(), rectangles.size());
        int i = 0, j;
        for (int k = 0, rectanglesSize = rectangles.size(); k < rectanglesSize; k++) {
            one.empty3.apps.feature.tryocr.Rectangle2 rect1 = rectangles.get(k);
            j = 0;
            for (int i1 = 0, size = rectangles.size(); i1 < size; i1++) {
                one.empty3.apps.feature.tryocr.Rectangle2 rect2 = rectangles.get(i1);
        ...
        HashMap<Integer, List<one.empty3.apps.feature.tryocr.Rectangle2>> matchingRects = new HashMap<>();
        for (int iSlides = 0; iSlides < nbValues; iSlides++) {
            ...
            for (int i1 = 0; i1 < index; i1++) {
                if (values[i1] >= min && values[i1] <= max) {
                    one.empty3.apps.feature.tryocr.Rectangle2 rectangle2 = rectangles.get(position[i1]);
    ...
    private void exec2(int i, int j) {
        ...
        if (arrayDiff(input.getValues(i, j), WHITE_DOUBLES) < MIN_DIFF) {
            ...
            if (succeded) {
                ...
                one.empty3.apps.feature.tryocr.Rectangle2 rectangle2 = new one.empty3.apps.feature.tryocr.Rectangle2(0, 0, 0, 0);
                if (reduce(input, new one.empty3.apps.feature.tryocr.Rectangle2(rectangle), rectangle2)) {
    ...
    private void exec3(int i, int j) {
        ...
        if (arrayDiff(input.getValues(i, j), WHITE_DOUBLES) < MIN_DIFF) {
            ...
            if (succeded) {
                Rectangle rectangle = new Rectangle(i, j, w, h);
                one.empty3.apps.feature.tryocr.Rectangle2 rectangle3 = new one.empty3.apps.feature.tryocr.Rectangle2(0, 0, 0, 0);
                if (reduce(input, new one.empty3.apps.feature.tryocr.Rectangle2(rectangle), rectangle3)) {
                    //rectangle2 = new Rectangle2(rectangle);// PROVISOIRE
    ...
    private List<Character> recognize(PixM input, one.empty3.apps.feature.tryocr.Rectangle2 rectangle2) {
    ...
    private void shakeCurves(State state, int choice) {
        switch (choice) {
            ...
            case ADD_RANDOM_CURVE:
                ...
                state.currentCurves.add(new CourbeParametriquePolynomialeBezier(new Point3D[]{one.empty3.apps.feature.tryocr.FeatureLine.getFeatLine(randomLine(), 0).multDot(Point3D.n(state.step, state.step, 0)).multDot(state.xyz), FeatureLine.getFeatLine(randomLine(), 1).multDot(Point3D.n(state.step, state.step, 0)).multDot(state.xyz)}));
    ...
    public boolean reduce(PixM input, one.empty3.apps.feature.tryocr.Rectangle2 rectangle2origin, Rectangle2 render) {
