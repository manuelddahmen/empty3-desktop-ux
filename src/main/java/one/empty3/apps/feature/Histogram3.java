        ...
        for (double i = -c.r; i < c.r; i++) {
            for (double j = -c.r; j < c.r; j++) {
                if (Math.sqrt((i) * (i) + (j) * (j)) < c.r * c.r
                        && c.x + i >= 0 && c.y + j >= 0 && c.x + i < m.getColumns()
 && c.y + j < m.getLines()) {
...
; i++) {
            for (int j = 0; j < m.getLines(); j++) {
                ...
                while (level.i >= iMin && level.i > iOrigin - 1.0 / numLevels && level.i < iOrigin + 1.0 / numLevels && level.r < Math.max(m.getColumns()
, m.getLines()) / 20.) {
                    level.r *= rFract;
                    getLevel(m, level);
                }
...
*/
            ...
            for (int i = 0; i < pointsOfInterest.size(); i++) {
                ...
                Color color = new Color(Lumiere.getIntFromFloats((float) (v), (float) (v), (float) (v)));
            ...
            // grouper les points par similarités et distances
              /*  group(pointsOfInterest);
                ...
                File fileToWrite3 = new File(directory.getAbsolutePath()
                        + "level"+ "_NEW_RGB.jpg");
                //fileToWrite.mkdirs();*/
            new one.empty3.libs.Image(new PixM(img2).normalize(0., 1.).getImage()).saveFile(out);
                /*
