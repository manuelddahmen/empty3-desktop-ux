...
import one.empty3.apps.gui.History;
import one.empty3.apps.gui.JTablePopUp;
import one.empty3.apps.gui.Main;
import one.empty3.apps.gui.ObjectDetailDescription;
import one.empty3.apps.gui.RETableRenderer;
import one.empty3.apps.gui.RPropertyDetailsRow;
import one.empty3.apps.gui.RepresentableEditor;
...
public class REditor extends JPanel implements PropertyChangeListener, RepresentableEditor {
    ...
    one.empty3.apps.gui.History history = new History();

    private one.empty3.apps.gui.RPropertyDetailsRow tableModel;
    private one.empty3.apps.gui.Main main;

    public REditor() {
    }

    public REditor(one.empty3.apps.gui.Main main, Representable re) {
        super();
        this.main = main;
        initComponents();
        init(re);
    ...
    public void init(Object re) {
        if (re != null) {
            if (re instanceof Representable) {
                ...
                this.tableModel = new one.empty3.apps.gui.RPropertyDetailsRow((Representable) re);
            ...
            } else if (re instanceof one.empty3.apps.gui.RPropertyDetailsRow) {
                labelBreadCumbs.setText(((one.empty3.apps.gui.RPropertyDetailsRow) re).getRepresentable().getClass().getSimpleName());
                one.empty3.apps.gui.RPropertyDetailsRow model = new one.empty3.apps.gui.RPropertyDetailsRow((one.empty3.apps.gui.RPropertyDetailsRow) re);
    ...
    private void tableObjectDetailsMouseClicked(MouseEvent e) {
        ...
        one.empty3.apps.gui.ObjectDetailDescription objectDetailDescription = tableModel.getObjectDetailDescriptions().get(selectedRow);
        if (e.getButton() == 1) {
            if (tableModel.getItemList(selectedRow) != null) {
                if (tableModel.getItemList(selectedRow) instanceof MatrixPropertiesObject) {
                    ...
                    java.awt.Color o = (Color) tableModel.getItemList(selectedRow);
    ...
    */
    ...
    private StructureMatrix getProperty() {
        one.empty3.apps.gui.ObjectDetailDescription description = tableModel.getObjectDetailDescriptions().get(tableObjectDetails.getSelectedRow());
    ...
    public one.empty3.apps.gui.JTablePopUp getTableObjectDetails() {
    ...
    private void initComponents() {
        ...
        tableObjectDetails = new one.empty3.apps.gui.JTablePopUp();
    ...
    public one.empty3.apps.gui.Main getMain() {
        return main;
    }
