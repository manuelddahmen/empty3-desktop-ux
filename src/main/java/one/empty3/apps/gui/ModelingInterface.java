...
public class ModelingInterface extends JFrame {
    ...
    private one.empty3.apps.gui.Tubulaire4map tubulaire4;
    ...
    public void init() {
        ...
        try {
            tubulaire4.texture(new TextureImg(new Image(ImageIO.read(new File("sauvegardes/WIN_20210622_09_55_55_Pro.jpg")))));
        } catch (IOException e) {
            e.printStackTrace();
            tubulaire4.texture(new ColorTexture(Color.WHITE.getRGB()));
        }
        ...
        Color color = new Color(Lumiere.getIntFromFloats(0.5f, 0.0f, 0.0f));
        g.setColor(color);
    ...
    public void refresh() {
        if (!runningViewDisplay)
            new Thread(() -> {
                ...
                zBuffer.texture(new ColorTexture(Color.WHITE.getRGB()));
                zBuffer.backgroundTexture(new ColorTexture(Color.WHITE.getRGB()));
                scene = new Scene();
                ...
                zBuffer.idzpp();
                zBuffer.draw();
                Image ecBufferedImage = zBuffer.image2();
    ...
    private void menuItemChooseColorActionPerformed(ActionEvent e) {
        ...
        if ((paintColor = new Color(JColorChooser.showDialog(this, "Choose paint color", paintColor))) != null) {
    ...
    private void draw(java.awt.Point p1, Point p2) {
        Graphics g = image.getGraphics();
