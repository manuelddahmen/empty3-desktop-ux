...
//                        if (getMain().getUpdateView().getView().getMeshEditorBean().isSelection()) {
                        ...
                        if (ZBufferImpl.INFINITY.equals(selectedPoint)) {
                            ...
                            switch (meshType) {
                                case one.empty3.apps.gui.MeshEditorBean.MESH_EDITOR_ParametricSurface:
                                    break;
                                case one.empty3.apps.gui.MeshEditorBean.MESH_EDITOR_Sphere:
                                    surface = new Sphere();
                                    break;
                                case one.empty3.apps.gui.MeshEditorBean.MESH_EDITOR_Cube:
                                    //surface = new Cube();
                                    break;
                                case one.empty3.apps.gui.MeshEditorBean.MESH_EDITOR_Plane:
    ...
    private void drawPoints(List<ModelBrowser.Cell> objects) {
        objects.forEach(cell -> {
            try {
                drawPoint((Point3D) cell.pRot, new Color(java.awt.Color.BLACK));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        });
    ...
    private void drawSelection() {
                ...
                try {
                    if (cell != null) {
                        if (getMain().getUpdateView().getzRunner().getLastImage() != null)
                            drawPoint((Point3D) cell, new Color(Color.RED));
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
        ...
        drawPoint(getMain().getUpdateView().getRuv(), new Color(Color.YELLOW));
    }
