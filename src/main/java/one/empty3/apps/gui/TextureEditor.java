...
import one.empty3.apps.gui.MyObservableList;
import one.empty3.apps.gui.TableModelTexture;
import one.empty3.apps.gui.UpdateViewMain;
...
public class TextureEditor extends JPanel {
        ...
        public class MyTransferHandler extends TransferHandler {
            ...
            public boolean importData(TransferSupport support) {
                ...
                if (Arrays.stream(extensionsImg).anyMatch(s -> s.equals(finalF.getAbsolutePath().substring(finalF.getAbsolutePath().lastIndexOf(".")).toLowerCase()))) {
                    try {
                        tableModelTexture.getLines().add(new one.empty3.apps.gui.TableModelTexture.ModelLine(f, new TextureImg(
                                new Image(ImageIO.read(f))), TextureImg.class));
                        done = true;
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                ...
                if (Arrays.stream(extensionsMov).anyMatch(s -> s.equals(finalF1.getAbsolutePath().substring(finalF1.getAbsolutePath().lastIndexOf(".")).toLowerCase()))) {
                    ...
                    tableModelTexture.getLines().add(new one.empty3.apps.gui.TableModelTexture.ModelLine(f, textureMov,
                            TextureMov.class));
                    done = true;
                }
    ...
    private one.empty3.apps.gui.MyObservableList<File> fileList = new one.empty3.apps.gui.MyObservableList<>();
    private Color currentColor;
    public int choiceTexType = 0;
    private one.empty3.apps.gui.TableModelTexture tableModelTexture;
    private Main main;
    ...
    public TextureEditor() {
        initComponents();
        tableModelTexture = new one.empty3.apps.gui.TableModelTexture();
        ...
        if (choiceTexType == 0) {
            ...
            if (retour == JFileChooser.APPROVE_OPTION) {
                File sel = choose.getSelectedFile();
                {
                    if (choiceTexType == 1) {
                        try {
                            TextureImg textureImg = new TextureImg(new Image(ImageIO.read(sel)));
                            getMain().getDataModel().addTexture(textureImg);
                            tableModelTexture.getLines().add(new one.empty3.apps.gui.TableModelTexture.ModelLine(sel, textureImg, textureImg.getClass()));
    ...
    private void initComponents() {
        ...
        button3 = new JButton();
        button4 = new JButton();
        updateViewMain1 = new one.empty3.apps.gui.UpdateViewMain();
        button2 = new JButton();
        button1 = new JButton();
        button5 = new JButton();
        scrollPane1 = new JScrollPane();
        table1 = new JTable();
        myObservableList1 = new one.empty3.apps.gui.MyObservableList<>();
