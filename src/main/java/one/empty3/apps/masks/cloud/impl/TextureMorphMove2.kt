...
class TextureMorphMove2(
    ...
    fun initTexture() {
        if(distanceAbClass==null) {
            ...
            if (distanceAbClass == null) throw RuntimeException("distanceAbClass is null")
            if (editData.pointsInModel != null && editData.pointsInModel!!.isNotEmpty() && editData.dimPictureBox != null) {
    ...
    override fun getColorAt(u: Double, v: Double): Int {
        ...
        if (editData.image != null) {
            ...
            if (editData.distanceAB is DistanceBezier3) ; else if (((editData.distanceAB!!.sAij == null)
                        || (editData.distanceAB!!.sBij == null))
                && !editData.distanceAB!!.javaClass.isAssignableFrom(
                    DistanceBezier3::class.java
                )
            ) {
            ...
            try {
                ...
                if (axPointInB != null) {
                    val p = Point3D(
                        axPointInB.x * (editData.image?.width?.toDouble() ?: 1.0),
                        axPointInB.y * (editData.image?.height?.toDouble() ?: 1.0),
                        0.0
                    )
                    
                    val currentDistanceAB = distanceAB
                    val currentConvexHull1 = editData.convexHull1
                    var percentB = 0.0
                    if (currentDistanceAB != null && currentConvexHull1 != null) {
                         percentB = distanceToConvexHull(currentDistanceAB.bDimReal, currentConvexHull1, p) + 10
                    }
                 ...
                 if (editData.distanceAB is DistanceProxLinear43 && (editData.distanceAB as DistanceProxLinear43).jpgRight != null) {
                        ...
                        var c : Point3D = dist4.findAxPointInBa13(u, v);
                            ...
                            /*&&editPanel.convexHull2!=null &&editPanel.convexHull2.isPointInHull(x1, y1)*/) {
                                ...
                                var k = 0
                                for(k in 0..2) {
                                    color[k] = (rgb1[k] + (rgb3[k] - rgb1[k]) * percentB*percent).coerceIn(0.0, 1.0);
                                }
...
*/
    ...
    private fun setDistanceAbClass2(pDistanceAbClass: Class<out DistanceAB?>) {
        ...
        val aDimReal = if (editData.image != null) Dimension(editData.image!!.width, editData.image!!.height) else Dimension(1, 1)
        val bDimReal =  editData.dimPictureBox ?: Dimension(1, 1)
    ...
    editData.convexHull2 = ConvexHull(
        ...
        if (editData.image != null && editData.points3 != null && pDistanceAbClass==DistanceProxLinear43::class.java && editData.imageFileRight != null)
        ...
        if (editData.image != null && editData.points3 != null && pDistanceAbClass==DistanceProxLinear44_2::class.java && editData.imageFileRight != null)
            editData.convexHull3 =
                ConvexHull(
                    editData.points3!!.values.stream().toList(),
                    Dimension(
        ...
        if (editData.image != null && editData.pointsInImage != null && !editData.pointsInImage!!.isEmpty())
            editData.convexHull1 = ConvexHull(
                editData.pointsInImage!!.values.stream().toList(), Dimension(
                    editData.image!!.width, editData.image!!.height
                )
            )
        if (editData.dimPictureBox != null && editData.pointsInModel != null && !editData.pointsInModel!!.isEmpty())
            editData.convexHull2 = ConvexHull(
        ...
        if (editData.pointsInImage != null && editData.pointsInModel != null) {
        ...
        if (editData.image != null && editData.model != null) {
            ...
            try {
                if (pDistanceAbClass==DistanceProxLinear1::class.java) {
                    ...
                    if (bDimReal != null && cDimReal != null && editData.imageFileRight != null) {
    ...
    /*
        ...
        if (editData.distanceAB != null && distanceAB != null) {
            Logger.getAnonymousLogger().log(
                Level.INFO,
                "distanceAb: " + editData.distanceAB!!.javaClass + " " + distanceAB!!.javaClass
            )
        } else {
             Logger.getAnonymousLogger().log(Level.WARNING, "distanceAb is null in setDistanceAbClass2")
        }
    ...
    fun loadTxt(index: Int, selectedFile: ByteArray?): HashMap<String?, Point3D?> {
        ...
        try {
            val split = selectedFile.toString().split("\n");
