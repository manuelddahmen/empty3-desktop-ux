...
public open class ImageProcessor3(
    ...
    override fun run() {
        try {
            ...
            if (!txt1.isNullOrEmpty()) editPolygonsMappings!!.loadTxtData(txt1, 0)
            if (!txt2.isNullOrEmpty()) editPolygonsMappings!!.loadTxtData(txt2, 1)
            if (!txt3.isNullOrEmpty()) editPolygonsMappings!!.loadTxtData(txt3, 2)
            ...
            while (true) {
                val zBufferImage = editPolygonsMappings!!.testHumanHeadTexturing.zBufferImage()
                val isRunning = editPolygonsMappings!!.isRunning
                val frame = editPolygonsMappings!!.testHumanHeadTexturing.frame()
                val maxFrames = editPolygonsMappings!!.testHumanHeadTexturing.maxFrames

                val isBlank = zBufferImage == null || isBlankImage(zBufferImage)
                
                if (!((editPolygonsMappings!!.testHumanHeadTexturing != null && isRunning && frame <= maxFrames && isBlank) || frame <= 3)) {
                    break
                }
