...
package one.empty3.apps.facedetect.dist_model_normalizable;

import one.empty3.apps.facedetect.*;
...
//                    .multDot(new Point3D(1.0 / right, 1.0 / bottom, 0.0))
    ...
    private Point3D nearLandmark(double u, double v) {
        ...
        uv = getFromModelUvTo01(uv);
        double distance = Double.MAX_VALUE;
        int indexI = -1, indexJ = -1;
        for (int i = 0; i < listBX.size(); i++) {
            for (int j = 0; j < listBY.size(); j++) {
                Point3D elem = surfaceB.getCoefficients().getElem(i, j);
                if (Point3D.distance(elem, uv) < distance) {
                    Point3D elem1 = getUvFrom01ToModelUv(elem);
                    indexI = (int) u;
                    indexJ = (int) v;
                    distance = Point3D.distance(elem, uv);
                }
            }
        }
        ...
        point3Dij = getUvFrom01ToModelUv(point3Dij);
