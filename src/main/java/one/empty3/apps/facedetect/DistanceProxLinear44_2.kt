...
class DistanceProxLinear44_2(
    ...
    aDimReal: Dimension2D?,
    bDimReal: Dimension2D,
    cDimReal: Dimension2D,
    opt1: Boolean,
    ...
    private val cDimReal: Dimension2D
    ...
    init {
        ...
        var cDimReal: Dimension2D = cDimReal
        this.C = C
        ...
        cDimReal = Dimension(jpgRight.getWidth(), jpgRight.getHeight())
            ...
            for (i in 0..<sizeA) {
                ...
                for (k in sizeIndexStart..<sizeA) {
                    ...
                    if (timeElapsed > computeTimeMax) { // 1 min max.
                        ended = true
                        Logger.getAnonymousLogger().severe("End of time DistanceProxLinear44_2")
                        break
                    }
                    if (k == i) continue
    ...
    private fun realSize(x: MutableList<Point3D?>, dim: Dimension) {
        for (i in x.indices) {
            x.set(
                i, x.get(i)!!.multDot(Point3D(1.0/dim.width, 1.0/dim.height, 0.0))
            )
        }
    }
