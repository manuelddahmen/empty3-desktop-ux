...
*/
    ...
    public PixM normalize(final double min, final double max) {
        ...
        for (int i = 0; i < image.getColumns()
; i++) {
...
; i++) {
    ...
    public double difference(PixM other, double precision) {
        if (precision == 0.0) {
            precision = Math.max(Math.max(this.columns
, other.getColumns()
),
                    Math.max(this.lines, other.getLines()));
        }
        double diff = 0.0;
        ...
        return diff / (this.columns
 * this.lines * other.getColumns()
 * other.getLines());
    }
