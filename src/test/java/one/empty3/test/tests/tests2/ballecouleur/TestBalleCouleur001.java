...
package one.empty3.test.tests.tests2.ballecouleur;
...
import one.empty3.test.tests.tests2.balleclou.BalleClous2;
...
public class TestBalleCouleur001 extends TestObjetSub {
    ...
    private final Color balleColor = new Color(java.awt.Color.BLUE);
...
//        ParametricSurface.getGlobals().setIncrV(0.1);
        ...
        LumierePonctuelle lumierePonctuelle = new LumierePonctuelle(Point3D.X, new Color(java.awt.Color.RED));
        lumierePonctuelle.setR0(1);

        scene().lumieres().add(lumierePonctuelle);

        lumierePonctuelle = new LumierePonctuelle(Point3D.Y, new Color(java.awt.Color.GREEN));
        lumierePonctuelle.setR0(1);

        scene().lumieres().add(lumierePonctuelle);

        for (int c = 0; c < Ncolors; c++) {
            map.put(new Point2D(Math.random() * 100, Math.random() * 100),
                    Color.newCol((float) Math.random(), (float) Math.random(), (float) Math.random()));
        }
    ...
    public void finit() {
        ...
        for (int j = 0; j < s.length; j++) {
            ...
            try {
                tubulaireN2.texture(new TextureImg(new Image(ImageIO.read(new File("samples/img/coeur.gif")))));
            } catch (IOException e) {
                e.printStackTrace();
            }
