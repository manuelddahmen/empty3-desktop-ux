...
/*
        ...
        list3.forEach(p3s -> {
            //one.empty3.libs.Color r = new Color((float) 1.0, (float) r(), (float) 1.0);
            p2.set(0, 1.0);
            p2.set(1, r());
            p2.set(2, 1.0);
            if (p3s.size() >= 2) {
...
/*
            ...
            Image bLines = new one.empty3.libs.Color(o.getColumns(), o.getLines(), one.empty3.libs.Color.TYPE_INT_RGB);
            for (LineSegment line : lines) {
                one.empty3.libs.Color ab = Color.random();
                if (line.getLength() >= 2) {
