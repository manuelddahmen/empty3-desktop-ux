...
import one.empty3.androidFeature.selection.Paste;
 ...
 * Segmentation
 */
public class Lines6 extends ProcessFile {
...
// remove long complicated uncertain loop//
        ...
        list3.forEach(point3DS -> {
            ...
            if (p1 != p2) {
             /*   bLines.getBitmap().drawLine((int) (double) p1.getX(),
                        (int) (double) p1.getY(),
                        (int) (double) p2.getX(),
                        (int) (double) p2.getY(), Color.random().toArgb());
...
*/
        ...
        one.empty3.feature.PixM stack = new PixM(
                pixM.getColumns(), pixM.getLines()
                //one.empty3.ImageIO.read(getStackItem(1))
        );
