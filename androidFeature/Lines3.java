...
//
        ...
        lists2.forEach(p3s -> {
            Color r = new Color(Lumiere.getInt(new double[]{(float) r(), (float) r(), (float) r()}));
            double xA;
            double yA;
            p3s.forEach(point3D -> {
                if (isInBound(point3D)) {
        ...
        list3.forEach(p3s -> {
            one.empty3.libs.Color r = new Color(Lumiere.getInt(new double[]{(float) r(), (float) r(), (float) r()}));
            double xA;
            double yA;
            /*p3s.forEach(point3D -> {
                if (isInBound(point3D)) {
                    img3.setValues((int) (double) (point3D.getX()), (int) (double) (point3D.getY()), r.red() / 255., r.green() / 255., r.blue() / 255.);
                }
        ...
        lists2.forEach(p3s -> {
            ...
            if (extremes[0][0] != null && extremes[0][1] != null && isInBound(extremes[0][0]) && isInBound(extremes[0][1])) {
                lines.add(new LineSegment(extremes[0][0], extremes[0][1], new ColorTexture(new Color(r))));
            }
...
*/
            ...
            if (pointsCurrent.size() > 2)
                lines.add(new LineSegment(extremes[1][0], extremes[1][1], new ColorTexture(new Color(r))));
        ...
        for (LineSegment line : lines) {
            ...
            if (line.getLength() >= 2) {
                ...
                if (isInBound(pDraw1) && isInBound(pDraw2)) {
                    bLines.setRgb(x1, y1, Color.WHITE);
                    bLines.setRgb(x2, y2, Color.WHITE);
                }
            }
        }

//            Image linesImg2 = new one.empty3.libs.Color(o.getColumns(), o.getLines(), one.empty3.libs.Color.TYPE_INT_RGB);
//
//
...
//            });
        ...
        one.empty3.Image.saveFile(img3.normalize(0.0, 1.0).getImage(), "jpg",
