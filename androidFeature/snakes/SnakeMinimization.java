...
%
% LITERATURE:
%  GMatrix. Kass, A. Witkin, D. Terzopoulos, "Snakes: Active Contour Models",
...
GMatrix Fext, :       external force field, 2D-2D vector image
%               for example: gradient, gvf, vfc
%  alpha :      elasticity parameter (membrane)
%  beta :       rigidity parameter (thin plate)
%  stepsz :     step size
...
/*        int N = (int) snake_in.size();
        ...
        double c = gamma * beta;
        GMatrix P = GMatrix.diag(GMatrix.repmat(new double[][]{{a}}, 1, N));
        P = P.plus(GMatrix.diag(GMatrix.repmat(new double[][]{{b}}, 1, N - 1), 1)).plus(GMatrix.diag(new double[][]{{b}}, -N + 1));
        P = P.plus(GMatrix.diag(GMatrix.repmat(new double[][]{{b}}, 1, N - 1), -1)).plus(GMatrix.diag(new double[][]{{b}}, N - 1));
        P = P.plus(GMatrix.diag(GMatrix.repmat(new double[][]{{c}}, 1, N - 2), 2)).plus(GMatrix.diag(new double[][]{{c, c}}, -N + 2));
        P = P.plus(GMatrix.diag(GMatrix.repmat(new double[][]{{c}}, 1, N - 2), -2)).plus(GMatrix.diag(new double[][]{{c, c}}, N - 2));
